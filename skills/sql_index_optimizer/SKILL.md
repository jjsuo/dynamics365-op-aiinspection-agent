...
### index_usage.csv
TableName / IndexName / UserSeeks / UserScans / UserLookups / UserUpdates / LastUserSeek

### fragmentation.csv
TableName / IndexName / FragmentationPct / PageCount / PageDensityPct

### missing_index.csv
TableName / EqualityColumns / IncludedColumns / AvgImpactPct / UserSeeks / PriorityScore

### table_size.csv
TableName / RowCount / TotalSizeMB / DataSizeMB / IndexSizeMB

### index_existing.csv（重叠检测用）
TableName / IndexName / IndexType / KeyColumns / IncludedColumns / IsPrimaryKey / IsUnique / IsCrmManaged
...
条件：AvgImpactPct > 50 且 (UserSeeks + UserScans) > 50

优先级：
- PriorityScore > 500,000 → P0（立即创建）
- 100,000–500,000 → P1
- < 100,000 → P2
...
IndexSizeMB > DataSizeMB × 2 → 索引过多或存在大量冗余
