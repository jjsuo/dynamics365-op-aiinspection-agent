...
当识别到高 CPU / 高 Disk Latency / 低 PLE 时，自动输出相关的 `BucketStart` 列表，提示用户：

```
⚠ 检测到 2026-05-08 14:25~14:30 期间 % Processor Time P95 = 92%
   可用 health_report 或 slow_sql_analyst 按 BucketStart=2026-05-08 14:25:00 下钻：
   → 查询同一时间桶 slowsql_5min_*.csv 的 TOP CPU SQL
   → 查询同一时间段 blocking_1425.csv 的 HeadBlocker
```
