...
阻塞分析不得只看阻塞本身。每一个 **Head Blocker** 与 **阻塞链** 都必须带着以下三类上下文判断。

### 1) 同期服务器资源（回答「阻塞引发的还是被引发的」）

| 数据源 | 用途 |
|---|---|
| `server_per_sql/<日>/perfmon_5min_*.csv` | 按 `HHmm`→`BucketStart` 取同期 CPU%、PLE、Disk sec/Read、Worker Threads、Memory Grants Pending |
| `memory/business_context.json` | 是否落在 `maintenance_windows`（预期内，如夜间备份）或 `peak_hours`（真异常） |
| `memory/sql_config.json` | `max_worker_threads` 与当前 worker 占用对比 → 是否 THREADPOOL 风险 |
| `memory/thresholds.json.sql_blocking` | `block_duration_ms_warning/critical`、`block_chain_depth_warning/critical`、`block_frequency_per_hour_warning` —— 所有告警阈值必须从此读取 |

**判断规则**：阻塞发生时若 CPU > `cpu_pct_critical` 或 PLE < `ple_seconds_critical` → 标 `🔥资源引发型`，否则是 `🗿资源正常但逻辑阻塞型`。

### 2) Head Blocker 要还是在跑什么（回答「元凶到底是谁」）

| 数据源 | 用途 |
|---|---|
| `slow_sql/<日>/slowsql_5min_*.csv` | 按 `BucketStart` + `executing_sql_text` 粗匹配；定位 Head Blocker 的 SQL 指纹，TotalCpuMs/ExecCount 帮助判断是批量作业还是日常查询 |
| `memory/d365_custom.json.batch_job_fingerprints` | 已知 batch 指纹命中 → 标注「预知 batch」降级为 P2 |
| `sql_index/<日>/missing_index.csv` | 若 Head Blocker SQL 的 WHERE 列出现在缺失索引 → 根因上升为「缺索引导致的长事务阻塞」 |

### 3) 阻塞表的规模与索引背景（回答「是索引缺失还是锁升级」）

| 数据源 | 用途 |
|---|---|
| `sql_index/<日>/table_size.csv` | 从 `executing_sql_text`/`waitresource` 解析出表名，反查 `RowCount_` 和 `UsedMB`；命中 `thresholds.json.table_size.row_count_huge` 打 `[BIG-TABLE]` |
| `sql_index/index_existing.csv` | 受阻塞表是否有覆盖 WHERE 列的索引；无 → 证实缺索引 |
| `memory/risk_profile.json.watch_list` | 阻塞表是否在 watch_list（AuditBase / AsyncOperationBase / POA）；是 → 属于跨巡检周期的持续性风险 |
...
- 分析其执行的 SQL 文本
- 判断是否为 D365 系统操作
- **★ 必在同一 `BucketStart` 的 `slowsql_5min_*.csv` 中回查 Head Blocker 的 SQL 指纹**，输出：ExecCount / TotalCpuMs / TopHostname，回答「元凶是一次重查还是批量刷」。命中 `d365_custom.json.batch_job_fingerprints` → 标注「已知 batch」。

### Step 3.5：★ 同期服务器资源快照（必填）

每次阻塞事件输出必须携带：
```
🔥 同期资源（2026-05-08 14:25，来自 perfmon_5min）：
  CPU: 87% (阈 85%) | PLE: 180s (阈 300s)
  Worker Threads: 420/512 | Grants Pending: 5
  Disk sec/Read: 22ms (阈 15ms)
  → 🔥 资源引发型：CPU + IO 双瓶颈放大了锁等待
⏰ 时间窗口判定：命中 peak_hours（2026-05-08 14:00-17:00）→ 真异常
```
若落在 `maintenance_windows` 内 → 标注「预期内」并降优先级。

### Step 3.6：★ 涉及表规模与索引背景（必填）

从 Head Blocker / Victim 的 `executing_sql_text`、`waitresource` 解析出所有表名，输出：
```
📋 涉及表：
  - AsyncOperationBase [BIG-TABLE]  8200万行 / 38GB  命中 watch_list
    └ index_existing.csv 中 (StatusCode, CompletedOn) 无覆盖索引 → 锁升级来自全扫
  - SystemUserBase                    1.2万行
```
**规则**：
- 大表 + 缺覆盖索引 → 阻塞事件一律升为 P1。
- 阻塞与 slow_sql、missing_index 的冲突点重合 → 输出「根治建议」调用 `sql_index_optimizer`。
...
文件名 `blocking_HHmm.csv` 的 `HHmm` 可直接映射到其他数据源的 5 分钟桶，这是阻塞分析反向驱动的关键能力：

| 目标 | 数据源 | 用途 |
|---|---|---|
| 同期 CPU/PLE/IO | `perfmon_5min_*.csv` 按 `BucketStart` | Step 3.5 已必填 |
| 同期 Top SQL | `slowsql_5min_*.csv` 按 `BucketStart` + CpuDesc | Step 3 Head Blocker 已必填 |
| 表规模 | `table_size.csv` 按表名 | Step 3.6 已必填 |
| 索引空白 | `index_existing.csv` + `missing_index.csv` | Step 3.6 已必填 |

如需跨源聚合报告→ 调用 `health_report` skill。
...
### 是否需要补充数据
跨源数据加载状态：
  - perfmon_5min_*.csv   ✓/✗  缺失 → 无法判断是资源引发型还是逻辑阻塞型
  - slowsql_5min_*.csv   ✓/✗  缺失 → 无法定位 Head Blocker 具体 SQL 指纹
  - table_size.csv       ✓/✗  缺失 → 无法识别大表背景
  - index_existing.csv   ✓/✗  缺失 → 无法校验是否缺覆盖索引
