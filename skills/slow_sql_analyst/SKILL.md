...
分析 **任何一条** 慢 SQL 都不得脱离以下三类上下文数据。若缺失，必须在输出的「数据补充提示」中明确标注。

### 1) 服务器资源快照（回答「当时服务器是否扛得住」）

| 数据源 | 用途 |
|---|---|
| `server_per_sql/<日期>/perfmon_5min_*.csv` | 按 `BucketStart` 取**同一 5 分钟**的 CPU%、PLE、Batch Requests/sec、Worker Threads、Memory Grants Pending、Disk sec/Read、Disk sec/Write |
| `sql_blocking/<日期>/blocking_*.csv` | 按 `BucketStart` 或时间区间判断同期是否伴随阻塞链（放大慢 SQL 影响） |
| `memory/sql_config.json` | 读取 `max_server_memory_mb` / `logical_cores` / `max_degree_of_parallelism`，评估 CPU%/PLE 是否触顶 |
| `memory/business_context.json` | 判断该时间点是否落在 `peak_hours`（真异常）或 `maintenance_windows`（预期内） |

读取方式：
```bash
python3 tools/data_reader.py --category server_per_sql --date <同日>
python3 tools/data_reader.py --category sql_blocking --date <同日>
```
...
| 数据源 | 用途 |
|---|---|
| `sql_index/<日期>/table_size.csv` | 取 SQL 涉及的每张表的 `TotalRows` / `TotalSizeMB` / `IndexSizeMB` / `PartitionCount` |
| `memory/thresholds.json → table_size` | `row_count_huge` (默认 1000万) / `size_gb_huge` (默认 50GB) 作为 huge 判定阈值 |
| `memory/risk_profile.json → watch_list` | AsyncOperationBase / AuditBase / PrincipalObjectAccess 是重点监控对象 |

**规则**：SQL 文本中出现的每张表，都必须反查 `table_size.csv`。命中 huge 阈值时在 TOP SQL 条目上打 `[BIG-TABLE]` 标签并列出行数、大小。

### 3) 已有索引 + 碎片状态（回答「建议的索引是否已经存在 / 是否要重建」）

| 数据源 | 用途 |
|---|---|
| `sql_index/index_existing.csv`（根目录静态文件） | 当前所有索引清单：IndexName / KeyColumns / IncludedColumns / IsUnique / FilterDefinition |
| `sql_index/<日期>/index_usage.csv` | user_seeks / user_scans / user_lookups / user_updates / last_user_seek / **avg_fragmentation_in_percent** |
| `memory/thresholds.json → index` | `fragmentation_pct_reorganize` (默认 10) / `fragmentation_pct_rebuild` (默认 30) |
| `memory/d365_custom.json → indexes` | `never_drop_patterns` / `d365_system_index_prefixes` 防止误建议删除系统索引 |

**强制校验流程**（任何索引建议必须全部走完）：

1. **已存在检查**：拟建索引的 `(表, 前导键列集合)` 是否已存在同构索引
   - 完全匹配 → 标 `[SKIP-EXISTS]`，不再生成 CREATE
   - 前缀匹配（键列是现有索引前缀） → 标 `[REUSE]`，给出复用建议
   - 不存在 → 标 `[NEW]`，生成 CREATE 脚本
2. **碎片检查**：若命中 REUSE 或 SKIP，查 `index_usage.csv` 的 `avg_fragmentation_in_percent`
   - `>= 30%` → 标 `[REBUILD]`，生成 `ALTER INDEX ... REBUILD WITH (ONLINE=ON)`
   - `>= 10% 且 < 30%` → 标 `[REORG]`，生成 `ALTER INDEX ... REORGANIZE`
   - `< 10%` → 无需维护
3. **使用率检查**：若拟建索引的列组合命中的现有索引 `user_seeks=0 AND user_scans=0 AND user_lookups>0`，说明键顺序有问题，需输出「调整键顺序」建议而不是新建
4. **系统索引保护**：命中 `d365_custom.json.indexes.never_drop_patterns` 或 `d365_system_index_prefixes` 的索引，只能建议「新增覆盖索引」，禁止建议 DROP
...
每条输出 **必须** 包含以下 9 个字段（缺省字段标注原因）：

1. 排名与危急等级（P1~P5）
2. SQL 类型识别
3. D365 关联判断（涉及哪个 D365 对象/功能）
4. **时间桶命中分布**：该指纹主要出现在哪几个 5 分钟桶
5. **★ 当时服务器资源（必填）**：取该 SQL 活跃的 TOP 3 桶来自 `perfmon_5min_*.csv` 的 CPU%、PLE、Batch Requests/sec、Worker Threads Used、Memory Grants Pending；同期是否有阻塞（来自 `blocking_*.csv`）。输出格式：
   ```
   🔥 当时服务器状态（2026-05-08 14:25 ~ 14:40）：
     CPU: 87% (超阈 85%) | PLE: 180s (超阈 300s)
     Batch/sec: 3200 | Workers: 420/512 | Grants Pending: 5
     同期阻塞链深度 3，最长阻塞 18s
     → 该 SQL 确实触发/加剧了资源瓶颈
   ```
   若时间点落在 `maintenance_windows` 内 → 标注「预期内」降低优先级。

6. **★ 涉及的表与行数（必填）**：从 SQL 文本解析出所有表名，反查 `table_size.csv`，输出格式：
   ```
   📋 涉及表：
     - PrincipalObjectAccess [BIG-TABLE]  8500万行 / 42GB / 无分区  ← 命中 watch_list
     - SystemUserBase                       1.2万行 / 15MB
   ```
   **当大表被涉及且未走索引**，一律升级为 P1。

7. **根因说明**：结合上面的资源状态 + 表规模鉴别，不得只描述 SQL 本身

8. **★ 索引建议（必填，走完四步校验）**：
   ```
   🔍 索引建议：
     [SKIP-EXISTS] IX_POA_ObjectId_Principal 已存在（键列完全匹配）
       └─ 碎片率 42% → [REBUILD]  生成重建脚本
     [NEW]         IX_SUB_StateCode_OwnerId_Incl  → 生成 CREATE
   ```

9. **预估收益**：结合表行数 + 现有索引使用情况给出量化预估（如「logical_reads 82万 → 预计降至 500 以下」）
...
为每条 TOP SQL 生成索引建议，每一条必须先经过 **跨源关联数据** 章节的四步校验。

标注类型：
- `[NEW]` 建议创建新索引（index_existing.csv 中找不到同构索引）
- `[SKIP-EXISTS]` 索引已存在（完全匹配），不生成 CREATE
- `[REUSE]` 已有索引可复用（前缀匹配），说明如何让 SQL 命中它
- `[REBUILD]` 碎片 >= 30%，生成 ALTER INDEX REBUILD
- `[REORG]`   10% <= 碎片 < 30%，生成 ALTER INDEX REORGANIZE
- `[REORDER]` 键顺序需调整（user_seeks=0 但 user_lookups>0）
- `[DROP-CANDIDATE]` 仅对 **非系统索引** 且 `user_seeks + user_scans + user_lookups = 0` 且 `user_updates > 0`，或 `d365_custom.json.indexes.allow_drop_pattern` 片配时才会出现
...
-- 生成时间：YYYY-MM-DD
-- 环境：PROD - SQL01
-- 校验来源：index_existing.csv + index_usage.csv + d365_custom.json
-- 注意：建议在业务低峰期执行，ONLINE=ON 减少锁影响
-- =============================================

-- [P1][NEW] sql_fingerprint: abc123
-- 表：ContactBase，优化 ownerid + statecode 过滤
-- 表规模：3200万行 / 18GB；index_existing.csv 无同构索引
...
-- [P1][SKIP-EXISTS][REBUILD] sql_fingerprint: def456
-- 表：PrincipalObjectAccess
-- 索引 IX_POA_ObjectId_Principal 已存在（键列完全匹配）
-- 碎片率 42% ≥ 30%，执行 REBUILD
ALTER INDEX [IX_POA_ObjectId_Principal]
ON [dbo].[PrincipalObjectAccess]
REBUILD WITH (ONLINE = ON, FILLFACTOR = 90, SORT_IN_TEMPDB = ON);
GO

-- [P2][REUSE] sql_fingerprint: ghi789
-- 表：AsyncOperationBase；拟建 (StateCode, OperationType) 属于现有 IX_Async_State_Type_Owner 的前缀
-- 索引已可复用，需开发侧确认 WHERE 顺序匹配前导列
-- 无需 DDL

-- [P2][REORG] sql_fingerprint: jkl012
-- 表：ActivityPointerBase
-- 索引 IX_Activity_ModifiedOn 碎片率 18%，在 [10,30) 区间
ALTER INDEX [IX_Activity_ModifiedOn]
ON [dbo].[ActivityPointerBase]
REORGANIZE;
GO
```

安全红线：
- **绝不输出 `DROP INDEX` 语句** 去删除匹配 `never_drop_patterns` 或 `d365_system_index_prefixes` 的索引
- 建议脚本必须包含 `WITH (ONLINE = ON)`，如 SQL 版本为 Standard 无法 ONLINE，标注 `-- WARNING: 非 Enterprise 需离线执行`
...
### 资源集中度摘要
TOP 3 SQL 占总 CPU：71%（高度集中型）
峰值时段：14:25-14:40（命中 peak_hours），同期 CPU 87% / PLE 180s
同期阻塞：有，深度 3
主要执行账号：CRM服务账号（Plugin触发）/ 报表账号
主要来源服务器：APP01（65%）

### TOP 10 慢 SQL 清单
[排名 + 等级 + 当时资源 + 涉及表 + 索引校验 + 根因 + 建议 + 收益]

### 趋势分析（多天时）
[趋势图表文本描述]

### 索引优化脚本（含校验标签）
[完整可执行 T-SQL，每条带 NEW/SKIP-EXISTS/REUSE/REBUILD/REORG 标签]

### 行动计划
[分优先级清单]

### 数据补充提示（若有缺失）
- 缺少 perfmon_5min_*.csv → 无法确认是否服务器手的住
- 缺少 table_size.csv → 无法识别大表
- 缺少 index_usage.csv → 无法判断碎片，所有建议回退到 [NEW/SKIP-EXISTS]两种
```
